...
void vga_clear_row(size_t row)
{
	for (size_t col = 0; col < VGA_WIDTH; col++) {
		*(volatile uintptr_t *)&VGA->buffer[row][col].ascii_code = ' ';
		*(volatile uintptr_t *)&VGA->buffer[row][col].color_code =
			VGA_COLOR_CODE;
	}
}
...
void vga_new_line(void)
{
	for (size_t row = 0; row < VGA_HEIGHT; row++) {
		for (size_t col = 0; col < VGA_WIDTH; col++) {
			*(volatile uintptr_t *)&VGA->buffer[row - 1][col]
				 .ascii_code = VGA->buffer[row][col].ascii_code;
			*(volatile uintptr_t *)&VGA->buffer[row - 1][col]
				 .color_code = VGA->buffer[row][col].color_code;
		}
	}
...
void vga_write_byte(uint8_t byte)
{
	...
	default:
		...
		*(volatile uintptr_t *)&VGA->buffer[row][col].ascii_code = byte;
		*(volatile uintptr_t *)&VGA->buffer[row][col].color_code =
			color;
		++VGA_COLUMN_POSITION;
		break;
...
void vga_write_string(const char *s, size_t len, ...)
{
	va_list args;
	va_start(args, len);
